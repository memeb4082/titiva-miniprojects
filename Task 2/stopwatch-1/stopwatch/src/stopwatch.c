...
volatile int tenths = 0, seconds = 0;

char timer0_string[30];
...
void Timer0IntHandler(void)
{
    ...
    usprintf(timer0_string, "    Timer: %d.%d    ", seconds, tenths);
    ...
    // usprintf(timer0_string, " %d ", timer0_counter);
    //
...
//
//*****************************************************************************
volatile uint8_t pb_debounced_state = 0x00000000;
...
int main(void)
{
    ...
    while (1)
    {
        if (seconds >= 20)
        {
            ...
            GrStringDrawCentered(&sContext, "20s limit reached", -1, 120, 220, false);
            if ((g_pui32ButtonPressed & USR_SW2) == USR_SW2)
            {
        ...
        // {
        // case NON_COUNTING:
        //     TimerDisable(TIMER0_BASE, TIMER_A); // Ensure the timer is off
        //     TimerDisable(TIMER1_BASE, TIMER_A); // Ensure the timer is off
        //     UARTprintf("counting %i\n", pressed);
        //     if (pressed == 1)
        //     {
        //         COUNTER = COUNTING;
        //     }
        ...
        //     break;
        // case COUNTING:
        //     TimerEnable(TIMER0_BASE, TIMER_A); // resume the timer
        //     TimerEnable(TIMER1_BASE, TIMER_A); // resume the timer
        //     UARTprintf("counting %i\n", pressed);
        //     // if(timer0_counter>Timer_Limit){
        //     //     //COUNTER = STOPPED;
        //     //     break;
        //     // }
        //     break;
        ...
        // UARTprintf("%i.%i", timer0_counter,timer1_counter);
        ...
        GrContextFontSet(&sContext, &g_sFontCm20);
        // GrStringDrawCentered(&sContext, timer0_string, -10, GrContextDpyWidthGet(&sContext) / 2, 8, 0);

        GrContextFontSet(&sContext, &g_sFontCm20);
        GrContextForegroundSet(&sContext, ClrWhite);
        GrStringDrawCentered(&sContext, "- Press SW1 to START", -1, 120, 60, false);
        GrStringDrawCentered(&sContext, "- Press SW1 again to PAUSE", -1, 120, 80, false);
        GrStringDrawCentered(&sContext, "- Press SW1 to RESUME", -1, 120, 120, false);
        GrStringDrawCentered(&sContext, "- Press SW2 to RESET", -1, 120, 140, false);

        char pressedChar[3];

        usprintf(pressedChar, " %i ", pressed);

        GrStringDrawCentered(&sContext, timer0_string, -1, 120, 200, true);
        GrStringDrawCentered(&sContext, pressedChar, -1, 120, 220, true);
    }
}
